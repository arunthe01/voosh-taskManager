...
function SignupComponent() {
  ...
  const [loginState,setLoginState] = useState({email:'',password:'',firstName:'',lastName:'',});
  const [errorState,setErrorState] = useState({isEmailInvalid:false, isPasswordInvalid:false,isFirstNameInvalid:false,isLastNameInvalid:false});
  ...
  const validateAndHandleLogin = ()=>{
   ...
   if(loginState.firstName.length == 0){
    setErrorState(prev => ({ ...prev, isFirstNameInvalid: true }));
   }else if(loginState.lastName.length == 0){
    setErrorState(prev => ({ ...prev, isLastNameInvalid: true }));
   }
    else if (!EmailRegex.test(loginState.email)) {
    ...
    <div className='flex justify-center mt-10' > 
        <div style={{width:'400px', borderWidth:'2.5px',boxShadow:'0px 0px 5px grey'}} className='p-4 border-solid  border-sky-500  rounded-md' >
            ...
            <div className='flex flex-col '>
              <input type="text" name="firstName" placeholder='First Name' className='border-0 p-2 my-1 bg-transparent outline-none border rounded-sm ' style={{boxShadow:'0px 0px 2px grey'}} onChange={changeHandler} value={loginState.firstName}/>
              <span className={`text-red-500 ${errorState.isFirstNameInvalid ? '':'hidden'}`}>First Name cannot be empty</span>
               <input type="email" name="lastName" placeholder='Last Name' className='border-0 p-2 my-1 bg-transparent outline-none border rounded-sm ' style={{boxShadow:'0px 0px 2px grey'}} onChange={changeHandler} value={loginState.lastName}/>
                <span className={`text-red-500 ${errorState.isLastNameInvalid ? '':'hidden'}`}>Second Name cannot be empty</span>
               <input type="email" name="email" placeholder='Email' className='border-0 p-2 my-1 bg-transparent outline-none border rounded-sm ' style={{boxShadow:'0px 0px 2px grey'}} onChange={changeHandler} value={loginState.email}/>
                ...
                <p className='flex justify-center my-2'>Already have an account? <Link to='/login' className='ml-1 text-blue-500'>Login</Link></p>
            </div>
        </div>
    </div>
  )
